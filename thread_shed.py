...
# first, we need to replace the artifact ;,;
...
for transaction in daily_transactions_split:
    for kur in transaction:
        transactions_clean.append(kur.replace("\n", "").strip(" "))
...
for i in range(len(transactions_clean)):
    double_customers.append(transactions_clean[::4])
    double_sales.append(transactions_clean[1::4])
    double_thread_sold.append(transactions_clean[2::4])
    break
# print(double_customers, double_sales, double_thread_sold)
...
# print(double_thread_sold)
thread_sold_split = []
for i in double_thread_sold:
    for thread in i:
...
# Define a function called color_count that takes one argument, color. The function should iterate through thread_sold_split and count the number of times the item is equal to argument, color.
...
for color in colors:
    print("We have sold {} threads from the color {}!".format(color_count(color), color))
